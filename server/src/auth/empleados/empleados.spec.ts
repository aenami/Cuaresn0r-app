...
function setup(opts: { usuarioAdmin?: { id_usuario: number } | null; adminsActivos?: number } = {}) {
  const { usuarioAdmin = { id_usuario: 1 }, adminsActivos = 1 } = opts;
  const spies = {
    empleadoFindUnique: jest.fn().mockResolvedValue({ id_empleado: 1, estado_empleado: 'ACTIVO' }),
    usuarioFindFirst: jest.fn().mockResolvedValue(usuarioAdmin),
    usuarioCount: jest.fn().mockResolvedValue(adminsActivos),
    empleadoUpdate: jest.fn((args: { data: Record<string, unknown> }) => Promise.resolve({ id_empleado: 1, ...args.data })),
  };
  const prisma = {
    empleado: { findUnique: spies.empleadoFindUnique, update: spies.empleadoUpdate },
...
describe('EmpleadosService.deactivate (proteccion del ultimo admin)', () => {
  it('409 al desactivar al empleado del unico admin activo', async () => {
    const { svc, spies } = setup({ usuarioAdmin: { id_usuario: 1 }, adminsActivos: 1 });
  ...
  it('desactiva al admin cuando queda otro admin activo', async () => {
    const { svc, spies } = setup({ usuarioAdmin: { id_usuario: 1 }, adminsActivos: 2 });
    await svc.deactivate(1);
    expect(spies.empleadoUpdate).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ estado_empleado: 'INACTIVO' }) }),
    );
  });
