...
const dto = (monto: number, metodo = 'EFECTIVO') => ({ monto, metodo }) as unknown as CreatePagoDto;
...
interface FacturaMock {
  ...
  pagos: { monto_total_pago: Prisma.Decimal; subtotal_pago: Prisma.Decimal; monto_servicio_pago: Prisma.Decimal }[];
}

function setup(facturaOverrides: Partial<FacturaMock> = {}, turno: unknown = { id_turno: 3 }) {
  const spies = {
    ...
    pagoCreate: jest.fn((args: { data: Record<string, unknown> }) => Promise.resolve({ id_pago: 1, ...args.data })),
...
describe('PagosService.registrar (guardas)', () => {
  it('exige un turno abierto', async () => {
    ...
    await expect(svc.registrar(1, 5, dto(50))).rejects.toBeInstanceOf(ConflictException);
  });
  ...
  it('no permite pagar una factura ya pagada', async () => {
    ...
    await expect(svc.registrar(1, 5, dto(50))).rejects.toThrow(/ya esta pagada/);
  });

  it('rechaza el sobrepago con 422', async () => {
    const { svc } = setup(); // total 119, sin pagos previos
    await expect(svc.registrar(1, 5, dto(200))).rejects.toBeInstanceOf(UnprocessableEntityException);
  });
});
...
describe('PagosService.registrar (prorrateo y cuadre)', () => {
  it('prorratea el pago parcial en subtotal/servicio/impuestos y suma el efectivo al turno', async () => {
    ...
    const data = spies.pagoCreate.mock.calls[0][0].data as Record<string, Prisma.Decimal>;
...
const dtoExc = (monto: number, excedente: number, destino?: string, metodo = 'EFECTIVO') =>
  ({ monto, metodo, excedente, destinoExcedente: destino }) as unknown as CreatePagoDto;
...
describe('PagosService.registrar (excedente)', () => {
  it('registra el excedente al saldar la cuenta y suma monto + excedente al cuadre de caja', async () => {
    ...
    const data = spies.pagoCreate.mock.calls[0][0].data as Record<string, Prisma.Decimal | string>;
    ...
    const inc = spies.turnoUpdate.mock.calls[0][0].data as { monto_cierre_esperado: { increment: Prisma.Decimal } };
  ...
  it('guarda el destino PROPINA cuando el cliente lo deja para los meseros', async () => {
    ...
    expect(spies.pagoCreate.mock.calls[0][0].data.destino_excedente_pago).toBe('PROPINA');
  });

  it('422 si el excedente llega en un pago que no salda la cuenta', async () => {
    const { svc } = setup(); // saldo 119
    await expect(svc.registrar(1, 5, dtoExc(50, 10, 'CASA'))).rejects.toThrow(/salda la cuenta/i);
  });

  it('422 si el excedente llega en un metodo distinto de efectivo', async () => {
    const { svc } = setup();
    await expect(svc.registrar(1, 5, dtoExc(119, 10, 'CASA', 'TARJETA'))).rejects.toThrow(/efectivo/i);
  });

  it('422 si hay excedente pero no se indica el destino', async () => {
    const { svc } = setup();
    await expect(svc.registrar(1, 5, dtoExc(119, 10, undefined))).rejects.toThrow(/destino/i);
  });
});
