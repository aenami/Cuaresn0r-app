...
function setup(opts: {
  ...
  const {
    pedido = { id_pedido: 7, tipo_pedido: 'LOCAL', estado_pedido: 'ABIERTO', fecha_cierre_pedido: null },
    ocupanteFicha = null,
    detalles = [],
  } = opts;
  const spies = {
  ...
  const tx = {
    ...
    detalleComanda: { findFirst: spies.dcFindFirst, findMany: spies.dcFindMany, update: spies.dcUpdate },
  };
  const prisma = {
...
describe('PedidosService.create', () => {
  it('abre un pedido LOCAL sin ficha ni mesa y crea su cuenta principal', async () => {
    ...
    expect(data).toMatchObject({ tipo_pedido: 'LOCAL', mesero_pedido: 5 });
    expect(data.id_ficha_pedido).toBeUndefined();
    expect(spies.subcuentaCreate).toHaveBeenCalledWith({
  ...
  it('conserva el flujo de domicilio con los datos de entrega', async () => {
    ...
    await svc.create(
      5,
      dto({ tipo: TipoPedido.DOMICILIO, nombreCliente: 'Juan', telefonoCliente: '3001112233', direccionCliente: 'Calle 1 #2-3' }),
    );
  ...
  it('asigna una ficha activa a un pedido local sin cerrarlo', async () => {
    ...
    expect(spies.pedidoFindFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ id_ficha_pedido: 4, estado_pedido: { notIn: ['CERRADO', 'CANCELADO'] } }),
      }),
    );
    expect(spies.pedidoUpdate).toHaveBeenCalledWith({ where: { id_pedido: 7 }, data: { id_ficha_pedido: 4 } });
  });

  it('409 si otro pedido aun activo ocupa la ficha', async () => {
    const { svc } = setup({ ocupanteFicha: { id_pedido: 99 } });
    await expect(svc.asignarFicha(7, 4)).rejects.toBeInstanceOf(ConflictException);
  });

  it('409 si se intenta asignar una ficha a domicilio', async () => {
    const { svc } = setup({ pedido: { id_pedido: 7, tipo_pedido: 'DOMICILIO', estado_pedido: 'ABIERTO' } });
    await expect(svc.asignarFicha(7, 4)).rejects.toBeInstanceOf(ConflictException);
  });
});
...
describe('PedidosService.cancel', () => {
  it('cancela las rondas pendientes y revierte solamente lo que ya preparaba', async () => {
    ...
    expect(spies.pedidoUpdate).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ estado_pedido: 'CANCELADO' }) }),
    );
  });
