...
import { EstadoPedido, Prisma, TipoPedido } from '../generated/prisma/client';
...
export class PedidosService {
  ...
  async create(idMesero: number, dto: CreatePedidoDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.subcuenta.create({
        data: { id_pedido_subcuenta: pedido.id_pedido, nombre_subcuenta: 'Cuenta principal' },
      });
  ...
  async asignarFicha(idPedido: number, idFicha: number) {
    return this.prisma.$transaction(async (tx) => {
      ...
      const fichas = await tx.$queryRaw<Array<{ id_ficha: number; ficha_activa: boolean }>>`
      ...
      if (!ficha.ficha_activa) throw new ConflictException('La ficha esta desactivada');

      const pedido = await tx.pedido.findUniqueOrThrow({ where: { id_pedido: idPedido } });
      if (pedido.tipo_pedido !== 'LOCAL') throw new ConflictException('Los domicilios no usan ficha');
      if (pedido.estado_pedido === 'CERRADO' || pedido.estado_pedido === 'CANCELADO') {
        throw new ConflictException(`No se puede asignar una ficha a un pedido ${pedido.estado_pedido.toLowerCase()}`);
      }
      ...
      if (ocupante) throw new ConflictException(`La ficha esta ocupada por el pedido #${ocupante.id_pedido}`);

      await tx.pedido.update({ where: { id_pedido: idPedido }, data: { id_ficha_pedido: idFicha } });
  ...
  async cancel(idPedido: number) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!pedido) throw new NotFoundException('Pedido no encontrado');
      if (pedido.estado_pedido === 'CERRADO') throw new ConflictException('No se puede cancelar un pedido cerrado');
      ...
      const facturaVigente = await tx.factura.findFirst({
        where: { subcuenta: { id_pedido_subcuenta: idPedido }, estado_factura: { not: 'ANULADA' } },
      });
      if (facturaVigente) {
        throw new ConflictException('Este pedido tiene facturas vigentes; anulalas antes de cancelarlo');
      }

      const hayEntregados = await tx.detalleComanda.findFirst({
        where: { comanda: { id_pedido_comanda: idPedido }, estado_dc: 'ENTREGADO' },
      });
      if (hayEntregados) throw new ConflictException('Ya se entregaron productos de este pedido; no se puede cancelar completo');

      const detalles = await tx.detalleComanda.findMany({
        where: { comanda: { id_pedido_comanda: idPedido }, estado_dc: { in: ['PENDIENTE', 'PREPARANDO'] } },
      });
