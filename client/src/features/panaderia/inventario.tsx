...
export function InventarioPanaderia({ esAdmin }: { esAdmin: boolean }) {
  ...
  async function crear() {
    if (!nombre.trim() || !Number.isInteger(Number(precio)) || Number(precio) < 0) return
    setGuardando(true)
    try {
      await bakeryApi.crearArticulo({ nombre: nombre.trim(), tipo, unidad, precioVenta: Number(precio) })
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div><h2 className="font-heading text-xl font-semibold uppercase">Existencias y conteo</h2>
        <p className="mt-1 max-w-2xl text-sm text-muted-foreground">Las hornadas y recepciones suman. El conteo compara lo que salio fisicamente con las ventas, traslados y mermas registrados.</p></div>
      {esAdmin && <Button size="sm" onClick={() => { setSeleccion(null); setCreando(true); setNombre(''); setPrecio('') }}><Plus className="mr-1 size-4" /> Articulo</Button>}
    </div>
    ...
    {creando && esAdmin && <form className="grid gap-3 rounded-lg bg-surface-high p-4 sm:grid-cols-2 lg:grid-cols-[1fr_11rem_8rem_9rem_auto] lg:items-end" onSubmit={(e) => { e.preventDefault(); void crear() }}>
      ...
      <div><Label htmlFor="pan-tipo-nuevo">Tipo</Label><Select value={tipo} onValueChange={(valor) => { setTipo(valor as TipoArticulo); if (valor !== 'INSUMO') setUnidad('UNIDADES') }}><SelectTrigger id="pan-tipo-nuevo"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="PANADERIA">Panaderia</SelectItem><SelectItem value="EXTERNO">Bebida / externo</SelectItem><SelectItem value="INSUMO">Insumo no vendible</SelectItem></SelectContent></Select></div>
      <div><Label htmlFor="pan-unidad-nuevo">Unidad</Label><Select value={unidad} onValueChange={(valor) => setUnidad(valor as UnidadInventario)}><SelectTrigger id="pan-unidad-nuevo"><SelectValue /></SelectTrigger><SelectContent>{(['UNIDADES', 'kg', 'g', 'dg', 'mg', 'ml', 'L'] as const).map((u) => <SelectItem key={u} value={u}>{u}</SelectItem>)}</SelectContent></Select></div>
      <div><Label htmlFor="pan-precio-nuevo">Precio COP</Label><Input id="pan-precio-nuevo" type="number" min="0" step="1" inputMode="numeric" value={precio} onChange={(e) => setPrecio(e.target.value)} /></div>
      <div className="flex gap-2"><Button type="submit" disabled={guardando || !nombre.trim() || !precio}>Crear</Button><Button type="button" variant="ghost" onClick={() => setCreando(false)}>Cancelar</Button></div>
    </form>}
